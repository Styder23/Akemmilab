...
// Consulta SQL para obtener las ventas en la fecha especificada y agrupar exámenes
$query = "
SELECT 
    c.idcomprobante, 
    c.total, 
...
WHERE DATE(c.fechacompro) = '$fecha'
GROUP BY c.idcomprobante, c.total, dv.Codigo, tc.tipo, tg.tipopag
...
class PDF extends FPDF {
    function Header() {
        ...
        $this->Cell(80);
        $this->Cell(30, 10, 'Reporte de Ventas por Día', 0, 0, 'C');
        $this->Ln(20);
        
        // Fecha de visualización del reporte
        $this->Cell(80);
        $this->SetFont('Arial', '', 12);
        $this->Cell(30, 10, 'Fecha: ' . $GLOBALS['fecha'], 0, 0, 'C');
        $this->Ln(10);
        ...
        $this->Cell(60, 10, utf8_decode('Exámenes'), 1, 0, 'C', true);
        $this->Ln();
    }
...
while ($row = $result->fetch_assoc()) {
    $pdf->Cell(30, 10, $row['idcomprobante'], 1, 0, 'C');
    $pdf->Cell(25, 10, $row['total'], 1, 0, 'C');
    $pdf->Cell(30, 10, $row['Codigo'], 1, 0, 'C');
    $pdf->Cell(25, 10, $row['tipo'], 1, 0, 'C');
    $pdf->Cell(25, 10, $row['tipopag'], 1, 0, 'C');
    $pdf->Cell(60, 10, $row['examenes'], 1, 0, 'C');
    $pdf->Ln();
}
