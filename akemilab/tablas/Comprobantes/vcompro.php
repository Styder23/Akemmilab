...
// Consulta base
$sql = "SELECT idcomprobante, fechacompro, total, destotal, tipo 
        FROM comprobante c 
        JOIN tipocomprobante t ON t.idtipocom = c.fk_tipocom";

// Obtener el total de registros sin filtro
$totalRecordsResult = mysqli_query($con, "SELECT COUNT(*) AS total FROM comprobante");
...
if (isset($_POST['search']['value']) && $_POST['search']['value'] != '') {
    ...
    $search_sql = " WHERE fechacompro LIKE '%" . $search_value . "%' 
                    OR tipo LIKE '%" . $search_value . "%'";
    $sql .= $search_sql;
}

// Obtener el total de registros filtrados
$filteredRecordsResult = mysqli_query($con, "SELECT COUNT(*) AS total FROM comprobante c 
                                             JOIN tipocomprobante t ON t.idtipocom = c.fk_tipocom 
                                             $search_sql");
$filteredRecordsCount = mysqli_fetch_assoc($filteredRecordsResult)['total'];

// Manejo de orden
$order_sql = " ORDER BY idcomprobante DESC"; // Orden por defecto
...
while ($row = mysqli_fetch_assoc($result)) {
    $data[] = [
        ...
        $row['destotal'],
        $row['tipo'],
        '<button class="btn btn-success btn-sm verBtn" data-idcomprobante="' . $row['idcomprobante'] . '">Ver</button> '.
        '<button class="btn btn-danger btn-sm delBtn" data-idcomprobante="' . $row['idcomprobante'] . '">Eliminar</button>'
