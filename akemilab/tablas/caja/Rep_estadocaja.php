...
$sqlCaja = "SELECT idcaja, montoini, importe FROM caja WHERE hora_cierre IS NULL ORDER BY hora_apertura DESC LIMIT 1";
...
if ($resultCaja->num_rows > 0) {
    ...
    // Obtener información del usuario y caja
    $sqlInfoCaja = "SELECT idcaja, dni, CONCAT_WS(' ', Nombre, Apellido) as Usuario_Caja, hora_apertura, hora_cierre, montoini, importe
                    FROM caja c 
    ...
    // Consultar los movimientos de la caja
    $sqlMovimientos = "SELECT fechacompro as fecha,concat_ws(' ','Comprobante: ', idcomprobante) as detalle, 'Ingreso' as tipo, total as monto FROM comprobante WHERE fk_idcaja = $idCaja
                       UNION ALL
                       SELECT fecha as fecha, concepto as detalle, 'Ingreso' as tipo, monto as monto FROM igre_egre WHERE fk_idcaja = $idCaja AND fk_idtipmovi = 1
                       UNION ALL
                       SELECT fecha as fecha, concepto as detalle, 'Egreso' as tipo, monto as monto FROM igre_egre WHERE fk_idcaja = $idCaja AND fk_idtipmovi = 2
                       ORDER BY fecha";
    ...
    die('No hay ninguna caja abierta actualmente.');
}
